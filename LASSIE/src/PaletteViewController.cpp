...
PaletteViewController::PaletteViewController(SharedPointers* _sharedPointers):
 // ProjectViewController* _projectView):
  ...
  markovModelLibButton("Markov Model Library") {
  ...
  pack_start(markovModelLibButton, Gtk::PACK_SHRINK);


  envelopeLibButton.signal_clicked().connect (
    sigc::mem_fun(*this, &PaletteViewController::showEnvelopeLibrary));

  markovModelLibButton.signal_clicked().connect(
    sigc::mem_fun(*this, &PaletteViewController::showMarkovModelLibrary));
...
void PaletteViewController::insertEvent(IEvent* _event){
  ...
  if (palette.get_selection()->get_selected() ==0) { //see if some row is selected
    ...
    else if (parent[columns.columnObjectName] == "Measurement"){
     windowToRefresh = projectView->meaWindow;
   }
  }
...
void PaletteViewController::showMarkovModelLibrary() {
  sharedPointers->mainWindow->showMarkovModelLibraryWindow();
}
